...
class CausalDDS_ModelTrainer(embedder):
    ...
    def __init__(self, args, train_df, test_df, gene_exp_dict, repeat, fold):
        ...
        self.model = CausalDDS(args, device = self.device, num_step_message_passing = self.args.message_passing, num_classes = self.num_classes, intervention = self.args.intervention, conditional = self.args.conditional).to(self.device)
    ...
    def train(self):
        ...
        #return self.test_auc, self.test_aupr, self.test_acc, self.test_kappa, self.test_bacc , self.test_f1,self.test_outputs
        return self.test_auc, self.test_aupr, self.test_acc, self.test_kappa, self.test_bacc , self.test_f1
    def explain(self):
    ...
    def embedding_visual(self,train_df):
        drug_embedding_dict = dict()
        indx = 0
        for bc, samples in enumerate(self.explain_loader):
            # if samples[3].detach().cpu().numpy()[0] == 1:
            masks = create_batch_mask(samples)
            ...
            drugA_features, drugB_features = self.model.interaction(_drugA_features, _drugB_features) #公式3 前一个是药物1 后一个是药物2
            lambda_pos_A, self.importance_A = self.model.compress(drugA_features) #3933 3933,1
            lambda_pos_A = lambda_pos_A.reshape(-1, 1) #3933,1
            lambda_neg_A = 1 - lambda_pos_A

                # Inject Noise for Drug A
            static_drugA_feature = drugA_features.clone().detach() #3933,600

            node_drugA_feature_mean = scatter_mean(static_drugA_feature, drugA.batch, dim = 0)[drugA.batch] #3933,600 计算节点的平均特征
            node_drugA_feature_std = scatter_std(static_drugA_feature, drugA.batch, dim = 0)[drugA.batch] #3933,600 计算节点特征标准差
            ...
            pos_drugA_s2s = self.model.set2set_pos_drugA(pos_drugA, drugA.batch) #128,1200


                # Inject Noise for Drug B

            lambda_pos_B, self.importance_B = self.model.compress(drugB_features)
            ...
            pos_drugB_s2s = self.model.set2set_pos_drugB(pos_drugB, drugB.batch)

            if train_df.loc[indx, "drug_a"] in drug_embedding_dict:
                drug_embedding_dict[train_df.loc[indx, "drug_a"]] = (drug_embedding_dict[train_df.loc[indx, "drug_a"]] + pos_drugA_s2s.cpu().detach().numpy()[0]) / 2
            else:

                drug_embedding_dict[train_df.loc[indx, "drug_a"]] = pos_drugA_s2s.cpu().detach().numpy()[0]

            if train_df.loc[indx, "drug_b"] in drug_embedding_dict:
                drug_embedding_dict[train_df.loc[indx, "drug_b"]] = (drug_embedding_dict[train_df.loc[indx, "drug_b"]] + pos_drugB_s2s.cpu().detach().numpy()[0]) / 2
            else:
                drug_embedding_dict[train_df.loc[indx, "drug_b"]] = pos_drugB_s2s.cpu().detach().numpy()[0]
            indx+=1
...
class CausalDDS(nn.Module):
    ...
    def __init__(self,
        ...
        #self.set2set_drug = Set2SetWithWeightedPooling(2 * node_hidden_dim, hidden_dim=4, num_iters=3)
    ...
    def compress(self, drugA_features):
        ...
        p = self.compressor(drugA_features) #公式4 3933X600 到 3933X1
        temperature = 1.0
    ...
    def forward(self, data, causal = False, test = False):
        ...
        cell_features = data[4]
        # node embeddings after interaction phase
        _drugA_features = self.gather(drugA) #公式2 节点个数,300 GIN得到的分子表征
        ...
        if causal == False:
        ...
        elif (test == True) or (causal == True):
            lambda_pos_A, self.importance_A = self.compress(drugA_features) #3933 3933,1
            lambda_pos_A = lambda_pos_A.reshape(-1, 1) #3933,1
            lambda_neg_A = 1 - lambda_pos_A
            ...
            pos_drugA_s2s = self.set2set_pos_drugA(pos_drugA, drugA.batch) #128,1200
            neg_drugA = global_mean_pool(neg_drugA, drugA.batch) #128,600
            ...
            if self.intervention == True:
                ...
                num = pos_drugA_s2s.shape[0] #128
                l = [i for i in range(num)]
                random.shuffle(l)
                random_idx = torch.tensor(l) #128X1

                if self.conditional == True:

                    # Create Intervention Interaction Map
                    rand_batch_drugA = random_idx[drugA.batch] #打乱后的索引，指示节点
                    ...
                    rand_drugA_drugB = torch.cat((pos_drugA_s2s, pos_drugB_s2s, neg_drugA[random_idx], neg_drugB[random_idx], cell_features), 1)
                    random_predictions = self.rand_predictor(rand_drugA_drugB)

...
class CMRL_ModelTrainer_reg(embedder):
    ...
    def __init__(self, args, train_df, test_df, gene_exp_dict, repeat, fold):
        ...
        self.model = CMRL_reg(args, device = self.device, num_step_message_passing = self.args.message_passing, num_classes = self.num_classes, intervention = self.args.intervention, conditional = self.args.conditional).to(self.device)

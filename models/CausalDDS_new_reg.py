...
from embedder_reg import embedder
...
class CausalDDS_ModelTrainer_reg(embedder):
    ...
    def __init__(self, args, train_df, test_df, gene_exp_dict, repeat, fold):
        ...
        self.model = CausalDDS_reg(args, device = self.device, num_step_message_passing = self.args.message_passing, num_classes = self.num_classes, intervention = self.args.intervention, conditional = self.args.conditional).to(self.device)
        self.optimizer = optim.Adam(params = self.model.parameters(), lr = self.args.lr, weight_decay = self.args.weight_decay)
        self.scheduler = ReduceLROnPlateau(self.optimizer, patience=10, mode='min', verbose=True) #学习率调节器
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            for bc, samples in enumerate(self.train_loader):
                ...
                loss_pos = loss_function_MSE(pos, samples[3].reshape(-1, 1).to(self.device).float()) #有效部分的loss
                
                #random_label = torch.normal(mean = 0.0, std = 1.0, size = neg.shape).to(self.device)
                #loss_neg = loss_function_MSE(neg, random_label)
                margin = 5.0
                loss_neg = F.relu(margin - F.mse_loss(pos, samples[3].reshape(-1, 1).to(self.device).float() , reduction='none') + F.mse_loss(neg, samples[3].reshape(-1, 1).to(self.device).float(), reduction='none')).mean()
                ...
                if self.args.intervention:
                    loss_inv = self.args.lam2 * loss_function_MSE(rand, samples[3].reshape(-1, 1).to(self.device).float())
                    loss += loss_inv
            ...
            self.scheduler.step(self.train_mse_score)
        ...
        #return self.test_auc, self.test_aupr, self.test_acc, self.test_kappa, self.test_bacc , self.test_f1,self.test_outputs,self.test_outputs
        return self.test_mse, self.test_rmse, self.test_pear, self.test_r2, self.test_mae , self.test_spear
    def explain(self):
...
class CausalDDS_reg(nn.Module):
    ...
    def __init__(self,
        ...
        super(CausalDDS_reg, self).__init__()

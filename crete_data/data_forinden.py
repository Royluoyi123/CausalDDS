...
if __name__ == "__main__":
    ...
    smiles_df_inden = pd.read_csv("DDS/ddsdata/AstraZeneca640/drug_smiles.csv",header=0)
    smiles_dict_inden = dict(zip(smiles_df_inden['Drug_name'],smiles_df_inden['Smiles']))
    #smiles_dict = dict(zip(smiles_df['pubchemid'],smiles_df['isosmiles']))
    df = pd.read_csv(f'DDS/ddsdata/DrugCombDB640/drug_synergy.csv', sep=",")
    df['synergy'] = np.where(df['synergy'] >= 10, 1, 0)
    processed_data, dataset = build_dataset(df, "drug_a", "drug_b","cell_line","synergy", smiles_dict)
    torch.save(processed_data, "DDS/ddsdata/AstraZeneca640/processed/train.pt")
    ...
    df = pd.read_csv(f'DDS/ddsdata/AstraZeneca640/independent_input_drug_cell.csv', sep=",")
    processed_data, dataset = build_dataset(df, "drug_a", "drug_b","cell_line","label", smiles_dict_inden)
    torch.save(processed_data, "DDS/ddsdata/AstraZeneca640/processed/test.pt")
